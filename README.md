1. All the requirements mentioned below are finished
2. Due to the very limited time (3 nights), Unit test is only done for one component, one action and one reducer
   Can TDD but didn't go for it due to limited time.
3. No time to refactor code in details and do performance optimisation and the error.
